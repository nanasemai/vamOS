...
ONEPLUS6_PATCHES_DIR="$DIR/kernel/patches/oneplus6"
TMP_DIR="$DIR/build/tmp-kernel-oneplus6"
OUT_DIR="$DIR/build"
BOOT_IMG=./boot-oneplus6.img

BASE_DEFCONFIG="defconfig"
CONFIG_FRAGMENT="$DIR/kernel/configs/oneplus6/oneplus6.config"

# 使用内核子模块中已有的设备树文件
COMMON_DTSI="arch/arm64/boot/dts/qcom/sdm845-oneplus-common.dtsi"
DTS_FILES=(
...
HOST_OS="$(uname)"
KERNEL_LINUX_VOLUME="vamos-kernel-linux-oneplus6"
...
echo "Starting vamos-builder container for OnePlus 6"
...
build_kernel() {
  ...
  if [ "$HOST_OS" = "Darwin" ]; then
    ...
    export CCACHE_DIR="$DIR/.ccache-oneplus6"
  fi
  ...
  export LOCALVERSION="-vamos-oneplus6-$GIT_REV"
  ...
  mv $BOOT_IMG "$OUT_DIR/"
  echo "-- Done! boot-oneplus6.img: $OUT_DIR/boot-oneplus6.img --"
  ls -lh "$OUT_DIR/boot-oneplus6.img"
}
...
install_dts() {
  ...
  for dts in "${DTS_FILES[@]}"; do
    ...
    if [ ! -f "$dst_dir/$dts_name" ]; then
      echo "Error: DTS file $dts_name not found in kernel submodule"
      exit 1
    fi
  done
...
ONEPLUS6_PATCHES_DIR='$ONEPLUS6_PATCHES_DIR'
TMP_DIR='$TMP_DIR'
OUT_DIR='$OUT_DIR'
BOOT_IMG='$BOOT_IMG'
